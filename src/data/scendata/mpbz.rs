...
impl MapTileDataSegment {
    ...
    pub fn test_against_raw_decomp(&self, info: &Option<ScenInfoData>, raw_decomp: &Vec<u8>) {
        log_write("Doing MPBZ recompilation test".to_owned(),LogLevel::DEBUG);
...
impl ScenSegment for MapTileDataSegment {
    fn compile(&self, info: &Option<ScenInfoData>) -> Vec<u8> {
        if info.is_none() {
            ...
            log_write(format!("Missing info parameter in MapTileDataSegment compiler"), LogLevel::FATAL);
            return Vec::new();
        }
    ...
    fn wrap(&self, info: &Option<ScenInfoData>) -> Vec<u8> {
        if info.is_none() {
            ...
            log_write(format!("Missing info parameter in MapTileDataSegment wrapper"), LogLevel::FATAL);
            return Vec::new();
        }
